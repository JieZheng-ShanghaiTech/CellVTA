...
class CellSegmentationInference:
    ...
    def __load_model(self) -> None:
        ...
        if self.run_conf["data"]["num_nuclei_classes"] == 6:
            self.COLOR_DICT = {
                1: [255, 0, 0],
                2: [34, 221, 77],
                3: [35, 92, 236],
        ...
        elif self.run_conf["data"]["num_nuclei_classes"] == 7:
            self.COLOR_DICT = {
                1: [255, 0, 0],
                2: [34, 221, 77],
                3: [35, 92, 236],
    ...
    def __get_model(
        ...
        if model_type == "CellViT":
            model_class = CellViT
        elif model_type == "CellViTUNIAdapter":
            ...
            for batch in tqdm.tqdm(
                ...
                for idx, (patch_instance_types, patch_metadata) in enumerate(
                    ...
                    for cell in patch_instance_types.values():
                        if cell["type"] == nuclei_types.get("Background") or cell["type"] == nuclei_types.get("background"):
                            continue
        ...
        if polygons:
            ...
            for cell_type in detected_types:
                ...
                cell_geojson_object["properties"]["classification"][
                    "name"
                ] = self.TYPE_NUCLEI_DICT[cell_type]
                cell_geojson_object["properties"]["classification"][
                    "color"
                ] = self.COLOR_DICT[cell_type]
