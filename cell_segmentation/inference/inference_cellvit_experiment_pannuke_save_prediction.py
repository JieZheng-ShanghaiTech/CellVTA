...
class InferenceCellViT:
    ...
    def __load_dataset_setup(self, dataset_path: Union[Path, str]) -> None:
        ...
        dataset_config_path = "/data/xijiexu/conic_cellvit_patient_x40_linear/dataset_config.yaml"
    ...
    def __check_eval_model(self) -> None:
        ...
        print(self.run_dir / "checkpoints" / self.checkpoint_name)
    ...
    def convert_to_type_map(self, inst_map, inst_type):
        type_map = np.zeros_like(inst_map)
        
        num_images = inst_map.shape[0]
        
        for i in range(num_images):
            id_list = inst_type[i].keys()
            for j in id_list:
                type_map[i][inst_map[i]==j] = inst_type[i][j]["type"]
    ...
    def run_patch_inference(
        ...
        with torch.no_grad():
            for batch_idx, batch in inference_loop:
                image_names, predictions, gt = self.inference_step(
                    model, batch, generate_plots=generate_plots
                )
                
                total_image_names += image_names
                
                inst_map = predictions.instance_map.detach().cpu().numpy()
                inst_type = predictions.instance_types
                
                type_map = self.convert_to_type_map(inst_map, inst_type)
                
                pred_map = np.concatenate([inst_map[:,:,:,np.newaxis], 
                                           type_map[:,:,:,np.newaxis]], axis=3)
                
                total_prediction.append(pred_map)
                
        total_prediction = np.concatenate(total_prediction, axis=0)
        np.save("/data/yangyang/bioLLMs/CellViT_Adapter_UNI/log/2025-01-30T215007_UNIAdapteronConicx40LinearPatientOpt_small/conic_cellvit_adapter_small_inst_x40_linear.npy",
                total_prediction)
        with open("/data/yangyang/bioLLMs/CellViT_Adapter_UNI/log/2025-01-30T215007_UNIAdapteronConicx40LinearPatientOpt_small/conic_cellvit_adapter_small_inst_x40_linear_image_names.json", "w") as f:
            f.write(json.dumps(total_image_names))
    ...
    def inference_step(
        ...
        return image_names, predictions, gt
