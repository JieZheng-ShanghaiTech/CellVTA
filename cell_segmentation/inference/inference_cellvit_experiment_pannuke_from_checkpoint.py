...
class InferenceCellViT:
    ...
    def __load_run_conf(self) -> None:
        ...
        """
        with open((self.config_path).resolve(), "r") as run_config_file:
    ...
    def __instantiate_logger(self) -> None:
        ...
        logger = Logger(
            ...
            log_dir=Path(self.output_dir).resolve(),
            comment="inference",
            use_timestamp=False,
            formatter="%(message)s",
        )
    ...
    def __check_eval_model(self) -> None:
        ...
        print(self.run_conf["model"]["path"])
        assert Path(self.run_conf["model"]["path"]).is_file()
    ...
    def setup_patch_inference(
        ...
        checkpoint = torch.load(
            self.run_conf["model"]["path"], map_location="cpu"
        )
        model = self.get_model(model_type=checkpoint["arch"])
        self.logger.info(
            f"Loading model from {str(self.run_conf['model']['path'])}"
        )
    ...
    def inference_step(
        ...
        if generate_plots:
            self.plot_results(
                ...
                outdir=Path(self.output_dir / f"inference_predictions_{self.run_conf['logging']['log_comment']}"),
                scores=scores,
            )
