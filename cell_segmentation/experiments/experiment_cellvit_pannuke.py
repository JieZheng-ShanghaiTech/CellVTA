...
class ExperimentCellVitPanNuke(BaseExperiment):
    ...
    def run_experiment(self) -> tuple[Path, dict, nn.Module, dict]:
        ...
        val_dataloader = DataLoader(
            val_dataset,
            batch_size=self.run_conf["training"]["batch_size"]*8,
            num_workers=16,
            pin_memory=True,
            worker_init_fn=self.seed_worker,
        )
    ...
    def get_transforms(
        ...
        if "resize".lower() in transform_settings:
            if transform_settings["resize"]:
                transform_list.append(A.LongestMaxSize(max_size=input_shape, interpolation=cv.INTER_LINEAR))
        ...
        if "resize".lower() in transform_settings:
            if transform_settings["resize"]:
                val_transform_list.append(A.LongestMaxSize(max_size=input_shape, interpolation=cv.INTER_LINEAR))
        val_transform_list.append(A.Normalize(mean=mean, std=std))
