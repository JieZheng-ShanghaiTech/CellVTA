...
from models.segmentation.cell_segmentation.vit_timm import TimmVisionTransformer
...
class CellViTUNI(CellViT):
    ...
    def forward(self, x: torch.Tensor, retrieve_tokens: bool = False) -> dict:
        ...
        out_dict["tissue_types"] = self.encoder(x)
        z0, z1, z2, z3, z4 = x, *self.extracted_features
    ...
    def create_UNI(self):
        ...
        # uni = timm.create_model(**uni_kwargs)
        uni = TimmVisionTransformer(img_size=224, patch_size=self.patch_size, init_values=1e-5, dynamic_img_size=True, 
                                    embed_dim=1024, depth=24, num_heads=16, num_classes=self.num_tissue_classes)
