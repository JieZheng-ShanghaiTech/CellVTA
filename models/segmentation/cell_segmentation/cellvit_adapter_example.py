...
class CellViT(nn.Module):
    ...
    def __init__(
        ...
        # not use    
        # self.decoder3 = nn.Sequential(
        #     Conv2DBlock(self.embed_dim, self.bottleneck_dim, 3, dropout=self.drop_rate),
        # )  # skip connection 3
    ...
    def _forward_upsample(
        ...
        """
        # print(branch_decoder)
        
        # print(z0.shape, z1.shape, z2.shape, z3.shape, z4.shape)
        b4 = branch_decoder.bottleneck_upsampler(z4) # 16->32 ==>  8->32 | 16 ->32
        # print(b4.shape)
        b3 = self.decoder3(z3) # 16->32  ==> 16 -> 32  | 32 => 32
        # print(b3.shape)
        b3 = branch_decoder.decoder3_upsampler(torch.cat([b3, b4], dim=1)) # 32 -> 64   1024 -> 256
        # print(b3.shape)
        b2 = self.decoder2(z2) # 16->64 ==> 32->64 | 64 ->64
        # print(b2.shape)
        b2 = branch_decoder.decoder2_upsampler(torch.cat([b2, b3], dim=1)) # 64->128 512->128
        # print(b2.shape)
        b1 = self.decoder1(z1) # 16->128  ==>  64->128 | 128 -> 128
        # print(b1.shape)
        b1 = branch_decoder.decoder1_upsampler(torch.cat([b1, b2], dim=1)) # 128->256  256->64
        # print(b1.shape)
        b0 = self.decoder0(z0) # 256->256 3->64
        # print(b0.shape)
        branch_output = branch_decoder.decoder0_header(torch.cat([b0, b1], dim=1)) # 256->256  x->num_classes
    ...
    def freeze_encoder(self):
        ...
        for layer_name, p in self.encoder.named_parameters():
            if (layer_name.split(".")[0] != "head") and (layer_name.split(".")[0] != "spm") and (layer_name.split(".")[0] != "interactions") \
                ...
                and (layer_name.split(".")[0] != "norm3") and (layer_name.split(".")[0] != "norm4"):  # do not freeze head
...
class CellViTSAMAdapter(CellViT):
    # under construction
    """CellViT_Adapter with SAM backbone settings
    ...
    def __init__(
        ...
        self.encoder = ViTCellViTDeitAdapter(
            extract_layers=self.extract_layers,
            depth=self.depth,
            embed_dim=self.embed_dim,
            mlp_ratio=4,
